...
if nnz(Num) > 0.05*nc
    ...
    % For empty segments create points by interpolation from neighboring non-empty
    % segments
    if any(Empty)
        for i = 1:nc
            if Empty(i)
        ...
        if length(CrossLen) == length(InterLines)
            ...
            for i = 1:2:m
                if InterLines(i) ~= n
                    ...
                    Curve(1,:) = Curve(InterLines(i),:)+0.9*CrossLen(i)/d(InterLines(i))*LineEle(InterLines(i),:);
                end
            end
            [Intersect,IntersectLines] = check_self_intersection(Curve(:,1:2));
            j = j+1;
