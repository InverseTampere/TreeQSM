...
if dim == 2 % 2d curves
    DirLines = [1./L.*V(:,1) 1./L.*V(:,2)]; % directions (unit vectors) of the line elements
    Intersect = false;
    if nargout == 1 % check only if the curve intersects
        ...
        for i = 1:n-1
            ...
            for j = ind
                ...
                if x(1) >= 0 && x(1) <= L(j) && x(2) >= 0 && x(2) <= L(i)
                    Intersect = true;
                    IntersectLines{i,1} = [IntersectLines{i,1}; j]; % which line elements cross element i
                    IntersectLines{j,1} = [IntersectLines{j,1}; i]; % which line elements cross element j
                    IntersectLines{i,2} = [IntersectLines{i,2}; x(1)]; % distances along element i to intersection points
                    IntersectLines{j,2} = [IntersectLines{j,2}; x(2)]; % distances along element j to intersection points
                end
            end
        end
...
elseif dim == 3 % 3d curves
    DirLines = [1./L.*V(:,1) 1./L.*V(:,2) 1./L.*V(:,3)]; % directions (unit vectors) of the line elements
    Intersect = false;
    if nargout == 1 % check only if the curve intersects
        while i <= n-1
            % Select the line elements that can intersect element i
            ...
            [~,DistOnRay,DistOnLines] = distances_between_lines(Curve(i,:),DirLines(i,:),Curve(I,:),DirLines(I,:));
            if any(DistOnRay >= 0 & DistOnRay <= L(i) & DistOnLines > 0 & DistOnLines <= L(I))
                Intersect = true;
                i = n;
            else
                i = i+1; % study the next line element
        ...
        for i = 1:n-1
            ...
            % Solve for possible intersection points
            [D,DistOnRay,DistOnLines] = distances_between_lines(Curve(i,:),DirLines(i,:),Curve(I,:),DirLines(I,:));
            if any(DistOnRay >= 0 & DistOnRay <= L(i) & DistOnLines > 0 & DistOnLines <= L(I))
                Intersect = true;
                J = DistOnRay >= 0 & DistOnRay <= L(i) & DistOnLines > 0 & DistOnLines <= L(I);
                ind = Ind(I);
                ind = ind(J);
